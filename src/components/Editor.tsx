...
export const Editor = memo(function Editor({ currentDocumentId }: EditorProps) {
  ...
  // 移除 displayContent 本地状态，直接使用全局状态以避免同步逻辑导致的无限循环和延迟
  ...
  useEffect(() => {
    if (currentDocumentId) {
      ...
      // autoSave.setCurrentDocumentId(currentDocumentId) // Simplified useAutoSave doesn't have this
    } else {
  ...
  // 错别字检查功能
  const spellCheck = useSpellCheck(state.editor.content, {
  ...
  // 使用全局内容进行预览
  const debouncedPreviewContent = useDebounce(state.editor.content, 300)
  ...
  // 移除 同步全局状态到显示内容 的 useEffect，因为现在直接使用 state.editor.content
  ...
  const StatusComponent = useMemo(() => (
    <div className="editor-status">
      ...
      <div className="status-center">
        {/* 统一的保存状态显示 */}
        <span className={`save-status-main ${autoSave.isSaving ? 'saving' : ''} ${autoSave.hasUnsavedChanges ? 'unsaved' : 'saved'}`}>
          {autoSave.isSaving ? (
            <>
      ...
      <div className="status-right">
        ...
        {autoSave.currentDocumentId && (
          <span className="status-item doc-id">
            🆔 {autoSave.currentDocumentId}
          </span>
        )}
